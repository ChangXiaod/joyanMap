 ...
 */
@WebServlet(name = "UpdateOrAddDataServlet", urlPatterns = {"/UpdateOrAddDataServlet"})
public class UpdateOrAddDataServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (data != null) {
                //将从前端获得数据转为json对象
                data_obj = JSON.parseObject(data);
            }
            //获取排除更新的字段数组
            JSONArray exclude_columns = null;
            String exclude_str = request.getParameter("excludes");
            System.out.println("[debug]exclude_str:" + exclude_str);
            if (exclude_str != null) {
                exclude_columns = JSON.parseArray(exclude_str);
            }
            //如果前端输入的数据参数不为空，执行数据单条更新操作
            if (data_obj != null) {
                //将数据更新到数据库表中
                int num = sdd.update_or_add(sid, data_obj, exclude_columns);
                if (num > 0) {
                    //记录操作日志                    
//                    if (info_obj != null) {
//                        sdd.addLog(info_obj.getIntValue("id"), info_obj.getString("userName"), info_obj.getString("realName"), CONSTANTS.REPLACE, data.toString(), sid, true);
//                    }
                    //增加数据成功
